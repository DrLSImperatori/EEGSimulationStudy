function generate_datasets_RIPLMFG_shuffle(ndatasets, numWorkers) %, dataset_string, interaction)
...
mkdir(['/home/laurai/matlab/EEGSimulationStudy/SimulationResults/Conn_BBCB_Results_RIPLMFG_shuffle2_' date_string '/' dataset_string{connkind}])
end
...
parfor idata = 1:ndatasets 
  Struct1=load(['/home/laurai/matlab/BBCB/truth_struct_riplmfg.mat']); 
  truth=Struct1.truth;
...
for snr =linspace(0.01,1,100)
  ...
  if idx==1
      truth.bandpass = [8 12]; %8 13% bandpass filter coefficients
      [b_band, a_band] = butter(3, truth.bandpass/fs*2);
  else
      truth.bandpass = [0.5 12]; %8 13% bandpass filter coefficients
      [b_band, a_band] = butter(3, truth.bandpass/fs*2);
  end
...
if connkind==1
    ...
    if idx==1
        ...
        Res.truth=truth;
    savetofile(['/home/laurai/matlab/Simulations/Conn_BBCB_Results_RIPLMFG_shuffle2_09032019/' dataset_string{1} '/dataset_' char(interactionkind(idx))  '_' num2str(snr_index) '_' num2str(idata) '.mat'], Res);
    else
        Res=struct();
        Res.wpli=wpli;
        Res.wsmi=wsmi;
    ...
    if idx==1      
        ...
        %Res.truth=truth;
    savetofile(['/home/laurai/matlab/Simulations/Conn_BBCB_Results_RIPLMFG_shuffle2_09032019/' dataset_string{2} '/dataset_' char(interactionkind(idx))  '_' num2str(snr_index) '_' num2str(idata) '.mat'], Res);
    else
        Res=struct();
        Res.wpli_f=wpli_f;
        Res.wsmi_f=wsmi_f;
