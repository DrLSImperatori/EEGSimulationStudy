function generate_datasets_LIPLMFG_shuffle(ndatasets, numWorkers) %, dataset_string, interaction)
...
mkdir(['/home/laurai/matlab/EEGSimulationStudy/SimulationResults/Conn_BBCB_Results_LIPLMFG_shuffle2_' date_string '/' dataset_string{connkind}])
end
...
for snr =linspace(0.01,1,100)
    ...
    if strcmp(interactionkind(idx),'linear')==1
        ...
        truth.sources_nonint = [shuffle(truth.sources_int(1,:)),shuffle(truth.sources_int(2,:))].';
    end
    ...
    % Determine truth.sources_int and truth.sources_nonint from referring to
    % other functions.
...
%     truth = rmfield(truth, 'sources_int');
    ...
    %mkdir(['/data3/Simulations/BBCB_Results_RIPLMFG_shuffle2_control_1000/' dataset_string{1} '/EEG/dataset_' char(interactionkind(idx)) '_' num2str(snr_index) '_' num2str(idata) '/']) %dataset_'  num2str(bandpassindex) '_' num2str(idata) num2str(snr_index)])  
    % save data 
    if idx==1
        Res=struct();
        Res.wpli=wpli;
        ...
        Res.truth=truth;
        savetofile(['/home/laurai/matlab/Simulations/Conn_BBCB_Results_LIPLMFG_shuffle2_09032019/' dataset_string{1} '/dataset_' char(interactionkind(idx))  '_' num2str(snr_index) '_' num2str(idata) '.mat'], Res);
    else
        Res=struct();
        Res.wpli=wpli;
        Res.wsmi=wsmi;
    ...
    if idx==1      
        ...
        savetofile(['/home/laurai/matlab/Simulations/Conn_BBCB_Results_LIPLMFG_shuffle2_09032019/' dataset_string{2} '/dataset_' char(interactionkind(idx))  '_' num2str(snr_index) '_' num2str(idata) '.mat'], Res);
    else
        Res=struct();
        Res.wpli_f=wpli_f;
        Res.wsmi_f=wsmi_f;
